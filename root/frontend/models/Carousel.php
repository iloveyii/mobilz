...
class Carousel extends CActiveRecord
{
    ...
    public function getCarousel() {
        ...
        foreach($cars as $car) {
            $carousel .= '<li data-target="#carousel-id" data-slide-to="'.$count.'"></li>';
            $count++;
        }
        $carousel .='</ol>
        ...
        foreach($cars as $car) {
            ...
            $carousel .=
                '<img alt="'.$car->alt.'" src="'. Yii::app()->baseUrl.'/images/carousel/'. $car->image_file . '" />
                <div class="container">
                    <div class="carousel-caption">
                        <h1>'.$car->heading.'</h1>
                        <p>'.$car->para.'</p>
                        <p><a href="#" class="btn btn-primary">Läs Mer</a></p>
                    </div>
                </div>
            </div>';
        }
        $carousel .= '</div>';
        $carousel .=
        '<a href="#carousel-id" class="left carousel-control bx-prev" data-slide="next">
            <span class="glyphicon glyphicon-chevron-left"></span>
        </a>
        <a href="#carousel-id" class="right carousel-control bx-next" data-slide="prev">
